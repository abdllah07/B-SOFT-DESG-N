        ...
        $oldPassword = md5($oldPassword);
            if($row['pass'] === $oldPassword) {
                if($newPassword == $newPasswordAgaing) {
                    ...
                    if($update_run) {
                        header("Location: indexProfileClient.php?notif= Password has been changed");
                        exit();
                    }
                    else{
                        header("Location: indexProfileClient.php?error= There is unknown problem");
                        exit();
                    }
                }
                else {
                    header("Location: indexProfileClient.php?error= The confirmation password does not match");
                    exit();
                }
            }else {
                header("Location: indexProfileClient.php?error= Old password is wrong");
                exit();
            }
