<?php
    ...
    if(isset($_POST['to']) && isset($_POST['title']) && isset($_POST['message-containt'])) {
        ...
        if(mysqli_num_rows($sql_run) > 0){
            ...
            if($query_run){
                header("Location: indexProfileClient.php?notif= Message sent successfully");
                exit();
            }else {
                header("Location: indexProfileClient.php?error= Message was not sent");
                exit();
            }

        }
        else {
            header("Location: indexProfileClient.php?error= There is not user name like this");
            exit();
        }
    }
