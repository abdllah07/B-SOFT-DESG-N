<?php
    ...
    if( isset($_POST['user-name']) && isset($_POST['first-name']) && isset($_POST['last-name']) && isset($_POST['birth-day']) 
        ...
        $all_certificates = validate($_POST['certificates']);
        $year_of_experience = validate($_POST['experience']);
        $languages = validate($_POST['languages']);
        $programming_languages = validate($_POST['programming_l']);
        ...
        if($query_run) {
            ...
            if(mysqli_num_rows($check_run) > 0) {
                    ...
                    if($query3_run){
                        $check2 = "SELECT *  FROM programmer WHERE user_no = $user_no";
                        $check2_run = mysqli_query($connection, $check2);
                        if(mysqli_num_rows($check2_run) > 0){
                            $query4 = "UPDATE programmer SET section = '$section', all_certificates = '$all_certificates', year_of_experience = '$year_of_experience',
                            languages = '$languages', programming_languages = '$programming_languages' where user_no = $user_no";
        ...
        // }
            ...
            header("Location: indexProfile.php?error= information did not updated");
            exit();
            }
